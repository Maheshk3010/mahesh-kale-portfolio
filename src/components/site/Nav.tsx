        ...
        <ul className="hidden items-center gap-5 lg:flex">
          {links.map(([href, label], i) => (
            <li key={href}>
              <a
                href={href}
                className="group font-mono text-[9px] font-bold uppercase tracking-[0.16em] text-muted-foreground transition-colors hover:text-primary"
              >
              ...
              <div className="mt-12 grid gap-1">
                {links.map(([href, label], i) => (
                  <SheetClose key={href} asChild>
                    <a
                      href={href}
                      className="grid grid-cols-[36px_1fr] items-center border-b border-border py-4 font-display text-xl font-bold uppercase"
                    >
