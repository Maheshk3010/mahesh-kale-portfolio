  ...
  return (
    <Section id="experience" eyebrow="Section 06 / Work log" title="Field experience" description="Three internship chapters. Responsibilities are shown as recorded; missing dates and outcomes remain unclaimed.">
      <div className="relative border-t border-border pt-8">
        <motion.div initial={reduced ? false : {scaleY:0}} whileInView={{scaleY:1}} viewport={{once:true}} transition={{duration:1.2}} className="absolute bottom-0 left-[17px] top-8 w-px origin-top bg-primary/60 sm:left-[91px]" />
        <div className="space-y-14">{items.map((exp,i)=><motion.article key={`${exp.company}-${exp.role}`} initial={reduced ? false : {opacity:0,x:-20}} whileInView={{opacity:1,x:0}} viewport={{once:true}} transition={{delay:i*.1}} className="relative grid gap-5 pl-12 sm:grid-cols-[64px_1fr] sm:pl-0">
