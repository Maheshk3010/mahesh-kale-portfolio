    ...
    >
      <div className="border border-border bg-surface">
        <div className="mis-console border-b border-border p-6 sm:p-8">
          <div className="mb-5 flex items-center justify-between gap-4 font-mono text-[9px] uppercase tracking-[0.14em]">
            <span className="text-muted-foreground">Operational data flow</span>
          ...
          <PipelineFlow
            compact
            label="MIS operations pipeline"
            steps={flow.map((label) => ({ label }))}
          />
        </div>
        <motion.div
          ...
          {misModules.map((module, index) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <Activity className="h-4 w-4 text-muted-foreground group-hover:text-primary group-focus-visible:text-primary" />
              </div>
              <h3 className="mt-8 font-display text-xl font-bold uppercase">{module.title}</h3>
              <p className="mt-4 text-sm leading-6 text-muted-foreground group-hover:text-foreground group-focus-visible:text-foreground">
                {module.detail}
              </p>
