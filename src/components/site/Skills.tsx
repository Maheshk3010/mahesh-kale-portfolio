...
export function Skills() {
  ...
  const active = capabilityGroups.find((group) => group.verb === activeSkill) ?? capabilityGroups[0];
          ...
          >
            ...
            <div className="mt-7 grid grid-cols-2 gap-px bg-border">
              {active.tools.map((tool) => (
                <div key={tool} className="min-h-16 bg-background p-3 font-mono text-[9px] font-bold uppercase">
                  {tool}
                </div>
              ))}
            </div>
            <div className="mt-8 border-l-2 border-success pl-5">
              <p className="font-mono text-[8px] uppercase tracking-[.14em] text-primary">Applied to</p>
